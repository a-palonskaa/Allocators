...
void set_min_warmup_time(double seconds) {
    benchmark()->min_warmup_time = (clock_t) seconds * CLOCKS_PER_SEC;
}
...
void set_max_testing_time(double seconds) {
    benchmark()->max_test_time = (clock_t) seconds * CLOCKS_PER_SEC;
}
...
void print_report(FILE* ostream) {
    fprintf(ostream, "\n-------------Testing results--------------\n\n");

    fprintf(ostream, "\t[Testing time]: %f\n\t[Tests amount]: %zu\n"
    ...
    fprintf(ostream, "----------------Warmup-------------------\n\n");
    fprintf(ostream, "\t[Warmup time]: %f\n\t[Warmup tests amount]: %zu\n",
